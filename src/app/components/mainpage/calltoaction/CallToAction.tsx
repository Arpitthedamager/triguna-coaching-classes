...
const CallToAction: React.FC = () => {
  return (
    <motion.div
      className="bg-gradient-to-r from-blue-900 to-black text-white py-8 px-6 md:px-52 rounded-lg flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0"
      initial={{ opacity: 0, y: 50 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true, amount: 0.2 }} // Triggers when 20% of the component is in view
      transition={{ duration: 0.8, ease: "easeOut" }}
    >
      {/* Text block with staggered animations */}
      <motion.div
        initial="hidden"
        whileInView="visible"
