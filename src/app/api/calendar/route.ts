  ...
  try {
    ...
    if (!className || !["9", "10", "11", "12"].includes(className)) {
      return NextResponse.json(
        {
          success: false,
          error: "Invalid or missing className. Use '9', '10', '11', or '12'.",
        },
        { status: 400 }
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Invalid data. Provide className and event with date and subjects.",
        },
        { status: 400 }
      );
    }
...
export async function DELETE(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!eventDate) {
      return NextResponse.json(
        { success: false, error: "Missing eventDate parameter." },
        { status: 400 }
      );
    }
    ...
    if (!result) {
      return NextResponse.json(
        {
          success: false,
          error: `No event found for class: ${className} with date: ${eventDate}`,
        },
        { status: 404 }
      );
    }
